...
def build_attention_html(snapshots: list[dict[str, object]]) -> str:
    ...
    for profile, summary, state, reason in attention:
        ...
        rows.append(
            f"""
<a class="attention-card attention-{tone}" href="?view=alerts&amp;resident={quote(resident_id, safe='')}" target="_self">
...
def build_resident_directory_html(snapshots: list[dict[str, object]]) -> str:
    ...
    for snapshot in ordered:
        ...
        cards.append(
            f"""
<a class="directory-card directory-{tone}" href="?view=resident&amp;resident={quote(resident_id, safe='')}" target="_self">
...
def build_global_alerts_html(snapshots: list[dict[str, object]]) -> str:
    ...
    for profile, alert in rows:
        ...
        cards.append(
            f"""
<a class="attention-card attention-{'critical' if severity == 'emergency' else 'warning'}" href="?view=alerts&amp;resident={quote(resident_id, safe='')}" target="_self">
  <span class="attention-marker" aria-hidden="true">!</span>
